...
class filterController extends Controller
{
    ...
    public function filterHight(Request $request)
    {
        ...
        $job = DB::table('employer_registers')->join('job', 'employer_registers.id', '=', 'job.employer_id')->join('employ_info', 'employer_registers.id', '=', 'employ_info.employ_id')->select('job.title', 'job.id', 'employ_info.name', 'employ_info.logo', 'employ_info.links', 'job.link', 'job.salary', 'job.address_job')->Where('job.title','LIKE', '%'. $datas .'%')->Where('job.address_job', $address)->Where('career','LIKE', '%'.$industry.'%')->Orwhere('employ_info.name','LIKE', '%'. $datas .'%')->Where('job.address_job', $address)->Where('career','LIKE', '%'.$industry.'%')->Orwhere('job.address_job', $address)->Orwhere('job.career', $industry)->get();
