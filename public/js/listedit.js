...
function updateResumeExperience(e) {
    ...
    var arrExperienceForm = $('#experience-form').serializeArray()
    arrExperienceForm.push({
        name: "csrf_token",
        value: $("#csrf_token").val()
    });
    $.ajax({
        type: "POST",
        url: ROOT_KIEMVIEC + mapKeywords.cv_hay + '/update-resume-experience',
        dataType: 'JSON',
        data: arrExperienceForm,
        success: function(data) {
            $("#csrf_token").val(data.csrf_token);
            if (data.csrf_token_error) {
                show_noti(2, data.csrf_token_error);
...
function editResumeTitle() {
    $.fancybox.open({
        'src': 'http://localhost:8000/user/my-profile/edit-resume-title',
        type: 'ajax',
        opts: {
            clickOutside: 'close',
            clickSlide: 'zoom'
